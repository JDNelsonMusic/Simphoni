...
from app.utils import prepare_configuration_context  # Import the helper function
...
def home():
    ...
    :return: Redirect to dashboard or render home.html.
    """
...
def dashboard():
    """
    User dashboard route. Displays configurations, custom models, conversations, and recent activity.

    :return: Render dashboard.html with context variables.
    """
    try:
...
def upload_model():
    ...
    :return: Redirect or render upload_model.html based on request method.
    """
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                file.save(file_path)
                logger.info(f"Uploaded model saved to {file_path}")
            except Exception as e:
                logger.error(f"Error saving uploaded model: {e}")
                flash("Failed to save the uploaded model.", "danger")
                return redirect(request.url)
...
def setup():
    ...
    :return: Redirect to dashboard or render setup.html based on form submission.
    """
    form = SetupForm()
    ...
    latest_config = (
        Configuration.query.filter_by(owner=current_user)
    ...
    if latest_config:
        try:
            ...
            logger.error(
                f"Invalid JSON in configuration.id={latest_config.id}: {latest_config.model_order}"
            )
        ...
        try:
            models_info = json.loads(latest_config.model_order)
        except json.JSONDecodeError:
            logger.error(
                f"Invalid JSON in configuration.id={latest_config.id}: {latest_config.model_order}"
            )
...
def view_configuration(config_id):
    ...
    :param config_id: ID of the configuration to view.
    :return: Render view_configuration.html with configuration details.
    """
    ...
    try:
        parsed_model_order = json.loads(configuration.model_order)
    except json.JSONDecodeError:
        flash("Invalid model order configuration.", "danger")
        parsed_model_order = []

    # Prepare context variables (similar to setup and edit_configuration)
    context = prepare_configuration_context(user=current_user, configuration=configuration)

    return render_template(
        "view_configuration.html",
        configuration=configuration,
        parsed_model_order=parsed_model_order,
        **context,
    )
...
def edit_configuration(config_id):
    ...
    :param config_id: ID of the configuration to edit.
    :return: Redirect to view_configuration or render edit_configuration.html.
    """
    configuration = Configuration.query.filter_by(id=config_id, owner=current_user).first()
    if not configuration:
        flash("Configuration not found.", "warning")
    ...
    # Prepare context variables using the helper function
    context = prepare_configuration_context(user=current_user, configuration=configuration)
...
def delete_configuration(config_id):
    ...
    :param config_id: ID of the configuration to delete.
    :return: Redirect to dashboard with a success or error message.
    """
    configuration = Configuration.query.filter_by(id=config_id, owner=current_user).first()
    if not configuration:
        flash("Configuration not found.", "warning")
