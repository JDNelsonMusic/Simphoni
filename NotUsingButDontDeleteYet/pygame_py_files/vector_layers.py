...
TOOLS = [TOOL_SELECT, TOOL_LINE, TOOL_RECT, TOOL_CIRCLE, TOOL_ELLIPSE, TOOL_POLYGON, TOOL_FREEHAND, TOOL_TEXT, TOOL_DELETE]
...
def get_mouse_pos_canvas(pos, zoom_level=1.0):
    x, y = pos
    return ((x - TOOLBAR_WIDTH) / zoom_level, y / zoom_level)
    ...
    def draw(self, surface):
        if self.type == 'line':
            ...
            if self.properties['fill']:
                fill_color = self.properties['fill_color'] + (self.properties['fill_opacity'],)
            ...
            if self.properties['fill']:
                fill_color = self.properties['fill_color'] + (self.properties['fill_opacity'],)
                pygame.draw.circle(surface, fill_color,
...
class Toolbar:
        ...
        if event.type == MOUSEBUTTONDOWN:
            ...
            for button in self.tool_buttons:
                if button['rect'].collidepoint(pos):
                    ...
                    if self.current_tool not in [TOOL_RECT, TOOL_ELLIPSE, TOOL_POLYGON, TOOL_TEXT]:
                        self.fill_enabled = False
                    return
    ...
    def handle_zoom(self, pos):
        # Define Zoom In button
        zoom_in_rect = pygame.Rect(self.rect.x + 10, self.rect.y + 10 + len(self.tools) * 50 + 20 + len(self.stroke_palette)*40 + 20 + len(self.fill_palette)*40 + 20 + 40, 30, 30)
        # Define Zoom Out button
        zoom_out_rect = pygame.Rect(self.rect.x + 50, self.rect.y + 10 + len(self.tools) * 50 + 20 + len(self.stroke_palette)*40 + 20 + len(self.fill_palette)*40 + 20 + 40, 30, 30)
        if zoom_in_rect.collidepoint(pos):
            self.zoom_level = min(3.0, self.zoom_level + 0.1)
        elif zoom_out_rect.collidepoint(pos):
    ...
    def render(self, surface):
        ...
        zoom_in_rect = pygame.Rect(self.rect.x + 10, palette_start_y + 40, 30, 30)
        zoom_out_rect = pygame.Rect(self.rect.x + 50, palette_start_y + 40, 30, 30)
        pygame.draw.rect(surface, LIGHT_GREY, zoom_in_rect)
        pygame.draw.rect(surface, LIGHT_GREY, zoom_out_rect)
        draw_text(surface, "+", (zoom_in_rect.x + 10, zoom_in_rect.y + 5), BLACK, font=BIG_FONT)
        draw_text(surface, "-", (zoom_out_rect.x + 10, zoom_out_rect.y + 5), BLACK, font=BIG_FONT)
        draw_text(surface, f"Zoom: {int(self.zoom_level * 100)}%", (self.rect.x + 10, palette_start_y + 80), BLACK, FONT)
...
class PropertiesPanel:
    def __init__(self, rect):
        ...
        self.stroke_width = 2
        self.fill_opacity = 255  # Opacity ranges from 0 (transparent) to 255 (opaque)
        ...
        if self.selected_object:
            if event.type == KEYDOWN:
                if event.key == K_UP:
                    ...
                    self.fill_opacity = min(255, self.fill_opacity + 10)
                    self.selected_object.properties['fill_opacity'] = self.fill_opacity
                elif event.key == K_LEFT:
                    self.fill_opacity = max(0, self.fill_opacity - 10)
                    self.selected_object.properties['fill_opacity'] = self.fill_opacity
    ...
    def render(self, surface):
        ...
        if self.selected_object:
            draw_text(surface, f"Selected: {self.selected_object.type}", (self.rect.x + 10, self.rect.y + 10),
                      color=BLACK, font=BIG_FONT)
            draw_text(surface, f"Stroke Width: {self.stroke_width}", (self.rect.x + 10, self.rect.y + 50),
                      color=BLACK, font=FONT)
            draw_text(surface, f"Fill Opacity: {self.fill_opacity}", (self.rect.x + 10, self.rect.y + 80),
                      color=BLACK, font=FONT)
            draw_text(surface, "Use Up/Down arrows to adjust Stroke Width", (self.rect.x + 10, self.rect.y + 110),
                      color=BLACK, font=FONT)
            draw_text(surface, "Use Left/Right arrows to adjust Fill Opacity", (self.rect.x + 10, self.rect.y + 140),
                      color=BLACK, font=FONT)
...
# Canvas Class
class Canvas:
    def __init__(self, rect, layers, history_manager, toolbar, grid_enabled=True, grid_size=20):
        self.rect = rect
        self.layers = layers
        ...
        self.zoom_level = toolbar.zoom_level
    ...
    def handle_event(self, event):
        if event.type == MOUSEBUTTONDOWN:
            ...
            if not self.rect.collidepoint(pos):
                return
            canvas_pos = get_mouse_pos_canvas(pos, self.zoom_level)
            ...
            elif self.toolbar.current_tool == TOOL_DELETE:
                self.delete_object_at(canvas_pos)
            else:
                self.drawing = True
            ...
            if self.drawing and self.current_tool != TOOL_SELECT:
                self.update_drawing(get_mouse_pos_canvas(event.pos, self.zoom_level))
            if self.dragging and self.selected_obj:
                self.move_object(get_mouse_pos_canvas(event.pos, self.zoom_level))
    ...
    def create_new_object(self, pos):
        props = {
            ...
            'fill_opacity': self.toolbar.fill_enabled and self.toolbar.fill_opacity or 255
        }
    ...
    def get_text_input(self, pos):
        input_box = pygame.Rect(self.rect.x + self.rect.width + TOOLBAR_WIDTH, pos[1], 200, 30)
        ...
        while not done:
            ...
            pygame.draw.rect(screen, WHITE, input_box)
            pygame.draw.rect(screen, BLACK, input_box, 2)
            draw_text(screen, user_text, (input_box.x + 5, input_box.y + 5), color=BLACK, font=FONT)
            pygame.display.flip()
        return user_text

    def render_grid(self, surface):
        for x in range(0, CANVAS_WIDTH, self.grid_size):
            pygame.draw.line(surface, LIGHT_GREY, (x, 0), (x, CANVAS_HEIGHT))
        for y in range(0, CANVAS_HEIGHT, self.grid_size):
            pygame.draw.line(surface, LIGHT_GREY, (0, y), (CANVAS_WIDTH, y))

    def render(self, surface):
        # Draw canvas background
        pygame.draw.rect(surface, WHITE, self.rect)

        # Create a subsurface for the canvas
        canvas_surface = surface.subsurface(self.rect)

        # Apply zoom
        zoomed_surface = pygame.transform.scale(canvas_surface, (int(CANVAS_WIDTH * self.zoom_level), int(CANVAS_HEIGHT * self.zoom_level)))

        # Fill with white background
        zoomed_surface.fill(WHITE)

        # Draw grid
        if self.grid_enabled:
            self.render_grid(zoomed_surface)

        # Draw all layers
        for layer in self.layers.values():
            layer.draw(zoomed_surface)

        # Blit the zoomed surface back to the main surface
        surface.blit(zoomed_surface, (self.rect.x, self.rect.y))
...
class VectorLayersApp:
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                if self.canvas.selected_obj:
                    ...
                    self.properties_panel.fill_opacity = self.canvas.selected_obj.properties.get('fill_opacity', 255)
                else:
    ...
    def export_image(self):
        export_surface = pygame.Surface((CANVAS_WIDTH, CANVAS_HEIGHT), pygame.SRCALPHA)
