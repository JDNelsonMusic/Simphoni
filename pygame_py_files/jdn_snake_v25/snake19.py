...
# Adjusted sizes to be approximately the same size as medium pumpkins
# Medium pumpkin size: width=50px, height=100px (since CELL_SIZE=5)
# Adjust monsters accordingly

# Ghost (square image), scale to 50x50 pixels
ghost_img = pygame.image.load('ghost6.png').convert_alpha()
ghost_img = pygame.transform.scale(ghost_img, (10 * CELL_SIZE, 10 * CELL_SIZE))

# Skeleton (2:5 aspect ratio), scale to width=40px, height=100px
skeleton_img = pygame.image.load('skeleton12.png').convert_alpha()
skeleton_img = pygame.transform.scale(skeleton_img, (8 * CELL_SIZE, 20 * CELL_SIZE))

# Zombie (9:16 aspect ratio), scale to width=56px, height=100px
zombie_img = pygame.image.load('zombie18.png').convert_alpha()
zombie_img = pygame.transform.scale(zombie_img, (9 * CELL_SIZE, 16 * CELL_SIZE))
...
def draw_monsters(monsters):
    ...
    for monster in monsters:
        pos = monster['pos']
        image = MONSTER_IMAGES[monster['type']]
        screen.blit(image, pos)

def spawn_food(snake_body1, snake_body2, size, image):
...
def spawn_monster(monster_type, snake_body1, snake_body2):
    """Spawn a monster at a random location."""
    image = MONSTER_IMAGES[monster_type]
    ...
    while True:
        ...
        collision = any(rect.collidepoint(segment) for segment in snake_body1 + snake_body2)
        if not collision:
            return {'type': monster_type, 'pos': pos, 'spawn_time': pygame.time.get_ticks(), 'size': size}
...
def main():
    ...
    for item in food_items:
        image = FOOD_IMAGES[item['type']]
        item['pos'] = spawn_food(snake1_body, snake2_body, None, image)
    ...
    total_game_time = 300  # 5 minutes in seconds
            ...
            if check_food_collision(snake1_pos, item):
                if item['type'] == 'light_apple':
                    ...
                    if item['type'] == 'small':
                        ...
                        monster_type = 'zombie'
                        damage = 18
                    monster = spawn_monster(monster_type, snake1_body, snake2_body)
                    monster['target'] = 'player2'
